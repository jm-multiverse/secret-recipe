...
function userFlow() {
    ...
    let unauthorizedResponse = http.get(`http://localhost:8100/api/unauthorized`, null, {
    headers: {
      'Content-Type': 'application/json',
      'Cookie': `accessToken=${accessToken}`
    },
    });
    let unauthorizedMessage = JSON.parse(unauthorizedResponse.body)
    prettyLog("Unauthorized Message: ", unauthorizedMessage)
    sleep(1)
}